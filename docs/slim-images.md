...
Slim-based images contain these pinned Node.js versions for AMD64 and ARM64:

- Node.js 20.20.2.
- Node.js 22.23.2.
- Node.js 24.19.0.

The default `node`, `npm`, `npx`, and `corepack` commands use Node.js 24. The complete installations are under `/opt/hostedtoolcache/node`. AMD64 installations use the `x64` tool-cache directory. ARM64 installations use the `arm64` directory. `/opt/acttoolcache/node` points to the same content.
...
On 2026-08-27, the AMD64 variants of all four Ubuntu 24.04 slim-based images were tested in Daytona container sandboxes. Each sandbox used 2 vCPU, 4 GiB of memory, and a 10 GB disk.
...
| `ubuntu-24.04:dind` | 1,619,832,832 bytes (1.51 GiB) | 0.84 s | Docker 28.3.3 started; a native scratch image built |
...
The maintained-image workflow runs a runtime smoke test for every published platform variant before it assembles the public tag.
